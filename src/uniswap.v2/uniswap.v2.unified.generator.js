...
const { getAvailableUniswapV2, getUniV2DataforBlockInterval, computeLiquidityUniV2Pool, computeUniswapV2Price, computeLiquidityUniV2PoolAvg } = require('./uniswap.v2.utils');
...
async function createUnifiedFileForPairAvg(endBlock, fromSymbol, toSymbol) {
    console.log(`${fnName()}: create/append for ${fromSymbol} ${toSymbol}`);
    const unifiedFilename = `${fromSymbol}-${toSymbol}-unified-data.csv`;
    const unifiedFullFilename = path.join(DATA_DIR, 'precomputed-avg', 'uniswapv2', unifiedFilename);
    let sinceBlock = 0;
    let toWrite = [];
    if(!fs.existsSync(unifiedFullFilename)) {
        fs.writeFileSync(unifiedFullFilename, 'blocknumber,price,slippagemap\n');
    ...
    for(const [blockNumber, data] of Object.entries(univ2Data)) {
        ...
        const price = computeUniswapV2Price(normalizedFrom, normalizedTo);
        for(let slippageBps = 50; slippageBps <= 2000; slippageBps += 50) {
            slippageMap[slippageBps] = computeLiquidityUniV2PoolAvg(normalizedFrom, normalizedTo, slippageBps/10000);
        }
...
// generateUnifiedFileUniv2(19_000_000);
