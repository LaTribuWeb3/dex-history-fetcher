...
const { getAvailableSushiswapV2, getSushiV2DataforBlockInterval, computeLiquiditySushiV2Pool, computeSushiswapV2Price, computeLiquiditySushiV2PoolAvg } = require('./sushiswap.v2.utils');
...
async function generateUnifiedFileSushiswapV2(endBlock) {
    ...
    for(const base of Object.keys(available)) {
        for(const quote of available[base]) {
            ...
            await createUnifiedFileForPairAvg(endBlock, base, quote);
        }
    }
...
async function createUnifiedFileForPairAvg(endBlock, fromSymbol, toSymbol) {
    console.log(`${fnName()}: create/append for ${fromSymbol} ${toSymbol}`);
    const unifiedFilename = `${fromSymbol}-${toSymbol}-unified-data.csv`;
    if(!fs.existsSync(path.join(DATA_DIR, 'precomputed-avg', 'sushiswapv2'))) {
        fs.mkdirSync(path.join(DATA_DIR, 'precomputed-avg', 'sushiswapv2'));
    }
    
    const unifiedFullFilename = path.join(DATA_DIR, 'precomputed-avg', 'sushiswapv2', unifiedFilename);
    let sinceBlock = 0;
    let toWrite = [];
    if(!fs.existsSync(unifiedFullFilename)) {
        fs.writeFileSync(unifiedFullFilename, 'blocknumber,price,slippagemap\n');
    ...
    for(const [blockNumber, data] of Object.entries(sushiv2Data)) {
        ...
        const price = computeSushiswapV2Price(normalizedFrom, normalizedTo);
        for(let slippageBps = 50; slippageBps <= 2000; slippageBps += 50) {
            slippageMap[slippageBps] = computeLiquiditySushiV2PoolAvg(normalizedFrom, normalizedTo, slippageBps/10000);
        }
...
// generateUnifiedFileSushiswapV2(19_000_000);
