...
const { getPriceNormalized, getSlippages, getSlippagesAvg } = require('./uniswap.v3.utils');
...
async function FetchUniswapV3HistoryForPair(pairConfig, fee, web3Provider, univ3Factory, currentBlock) {
    ...
    const latestDataFilePath = `${DATA_DIR}/uniswapv3-avg/${pairConfig.token0}-${pairConfig.token1}-${fee}-latestdata.json`;
    ...
    const dataFileName = `${DATA_DIR}/uniswapv3-avg/${token0.symbol}-${token1.symbol}-${fee}-data.csv`;
...
function getSaveData(token0, token1, latestData) {
    ...
    const slippages = getSlippagesAvg(latestData.currentTick, latestData.tickSpacing, latestData.currentSqrtPriceX96.toString(), latestData.ticks, token0.decimals, token1.decimals);
    const saveValue = {
        p0vs1: p0,
        p1vs0: 1/p0
    };
